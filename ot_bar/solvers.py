...
def solve_OT_barycenter_fixed_point(X, Y_list, p, b_list, cost_list, B,
    ...
    try:
        for it in iterator:
            ...
            for k in range(K):
                ...
                tempX = nx.copy(X)
                tempY = nx.copy(Y_list[k])
                # if k == 0:
                    # tempX_list.append(tempX)
                    # tempY_list.append(tempY)
                # if it == 0:
                #     _, newTempY = add_projections(tempX[:n_init], tempY, p)
                # else:
                #     _, newTempY = add_projections(tempX, tempY, p)
                # tempX_list[0] = tempX
                # else:
                #     tempX, newtempY = add_projections(tempX, tempY, p)
                #     newtempX, _ = add_projections(tempX_list[0], tempY, p)
                #     tempX_list[0] = newtempX
                # tempX_list.append(newTempX)

                # tempY_list.append(newtempY)
                tempX_list.append(tempX)
                tempY_list.append(tempY)
                # tempY_list.append(newTempY)
                # print(tempY)
            # X_prev = nx.copy(tempX_list[0])
            # tempX_list = tempX_list[0] * K 
            ...
            for k in range(K):
                ...
                # print(tempX.shape[0])
                # print(tempY.shape[0])
                # add_projections(tempX, tempY)
                # b_list_temp = TT(ot.unif(tempY.shape[0]))
                # array = getattr(nx , "asarray", None)
                b_temp = torch.asarray(ot.unif(tempY.shape[0]))
                # n = tempX.shape[0]
                # n_list.append(n)
                a = nx.from_numpy(ot.unif(n), type_as=tempX)
                # print("ALLO")
                pi_list.append(ot.emd(a, b_temp, cost_list[k](tempX, tempY)))
                # print("ALLO2")
            Y_perm = []
            for k in range(K):
                # Y_perm.append(n_list[k] * pi_list[k] @ tempY_list[k])
                # print(pi_list[k].shape)
                # print(tempY_list[k].shape)
                Y_perm.append(n * pi_list[k] @ tempY_list[k])
            X = B(Y_perm)
            if log:
                X_list.append(X)
